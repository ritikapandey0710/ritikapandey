...
BEGIN
  -- Today's date in the server's local timezone (Asia/Calcutta)
  v_today := (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Calcutta')::date;
  ...
  v_start_date := ((v_today - INTERVAL '29 days')::timestamp AT TIME ZONE 'Asia/Calcutta') AT TIME ZONE 'UTC';
  ...
  LEFT JOIN (
    SELECT ("createdAt" AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Calcutta')::date AS day, COUNT(*) AS count
    FROM "Ticket"
