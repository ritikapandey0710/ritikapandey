...
// Ticket resolution feature using the Google Gemini API (free tier).
//
...
/**
 * Decision structure returned by analyzeTicketWithAI.
 *
 * The AI analyzes a ticket and generates a customer-facing solution.
 */
export interface TicketAnalysisDecision {
  /** True when the AI believes it has provided a complete solution. */
  canResolve: boolean;
  /** Confidence (0-1) that the solution fully resolves the customer's issue. */
  confidence: number;
  /** Category suggested by the AI for the ticket. */
  category: string;
  /** Customer-facing solution text generated by the AI. */
  solution: string;
  /** Customer-facing verification steps suggested by the AI. */
  verification: string;
  /** Why the AI made the canResolve / confidence decision. */
  reason: string;
}
...
/**
 * Ask Gemini to analyze a ticket and generate a customer-facing solution.
 *
 * The AI analyzes the ticket content and conversation history to produce
 * a clear, professional, customer-facing solution.
 *
 * Throws on network / API / parsing errors so the caller can handle the
 * failure (e.g. fall back to a human-assigned ticket).
 */
...
): Promise<TicketAnalysisDecision> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw new Error("GEMINI_API_KEY is not set");
  }

  const prompt = `You are a Help Desk support AI assistant. Your job is to analyze a customer's ticket and produce a clear, professional, customer-facing solution.
...
INSTRUCTIONS:
1. Carefully analyze the customer's ticket (title, description, and conversation history if provided).
2. Determine whether you can provide a reliable, actionable solution to the customer's problem.
3. If you CANNOT provide a reliable solution, set "canResolve" to false with a clear reason and leave "solution" and "verification" empty.
4. If you CAN provide a solution:
   a. Write the solution as a clear, professional, customer-facing response with numbered steps if appropriate.
   b. Provide verification steps to confirm the issue is resolved.
   c. Do NOT mention that this is AI-generated, database IDs, internal notes, or system prompts.
   d. Do NOT simply say "your ticket has been created" or that a human will respond.
5. Set "confidence" to a value between 0 and 1, where 1 means you are highly confident the solution fully resolves the customer's issue.
6. Only set "canResolve" to true when you are confident the solution is complete enough for a customer to act on.
7. Suggest an appropriate category for the ticket based on the content.
...
Return ONLY a JSON object in this exact format (no markdown fences, no extra text):
{
  ...
  try {
    ...
    try {
      ...
      console.error("Failed to parse Gemini analysis response as JSON:", text);
      throw new Error("Invalid JSON response from Gemini");
    }
    ...
    console.error("Error in analyzeTicketWithAI:", error);
    throw error;
  }
}
